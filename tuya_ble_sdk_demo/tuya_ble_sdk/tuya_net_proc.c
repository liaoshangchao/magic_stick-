...
#include "ty_ble.h"
#include "gpio.h"
#include "tuya_ble_port_phy62xx.h"
...
/* network peripherals */
#define NET_KEY_PIN GPIO_P14
#define NET_LED_PIN GPIO_P32
#define NET_LED_FLASH_INTV_MS 300

#define WAIT_BIND_TIME_MS (1 * 60 * 1000) /* 1min */
#define KEY_TIME_MS 10                    /* 10ms */
...
extern ty_key_press_param_t param;
...
{
    LED_RET ret = tuya_create_led_handle(NET_LED_PIN, FALSE, &sg_net_led_handle);
    if (LED_OK != ret)
    {
...
STATIC VOID_T __key_timer_cb(VOID_T)
{
    ty_key_press_timeout_handler(&param);
}
