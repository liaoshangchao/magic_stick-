 ...
 * @return none
 */
STATIC VOID_T __key_gpio_init(_IN CONST gpio_pin_e port, _IN CONST BOOL_T active_low)
{
    // hal_gpio_pin_init(port,GPIO_INPUT);
...
STATIC BOOL_T __get_key_stat(_IN CONST gpio_pin_e port, _IN CONST UCHAR_T active_low)
{
    ...
    {
        key_stat = hal_gpio_read(port) == 0 ? FALSE : TRUE;
    }
    return key_stat;
}
