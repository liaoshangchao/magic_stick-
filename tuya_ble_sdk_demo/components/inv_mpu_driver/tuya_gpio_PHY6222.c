 ...
 * @return none
 */
VOID_T __gpio_irq_handler(gpio_pin_e pin, gpio_polarity_e action)
{
...
GPIO_RET tuya_gpio_irq_init(_IN CONST TY_GPIO_PORT_E port, _IN CONST TY_GPIO_IRQ_TYPE_E trig_type, _IN TY_GPIO_IRQ_CB irq_cb)
{
    ...
    case TY_GPIO_IRQ_RISING:
        hal_gpio_pull_set(port, GPIO_PULL_DOWN);
        hal_gpioin_register(port, irq_cb, NULL);
        //        in_config.pull = NRF_GPIO_PIN_PULLDOWN;
        //        in_config.sense = NRF_GPIOTE_POLARITY_LOTOHI;
        break;
    case TY_GPIO_IRQ_FALLING:
        hal_gpio_pull_set(port, GPIO_PULL_UP);
        hal_gpioin_register(port, NULL, irq_cb);
