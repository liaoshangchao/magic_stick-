 ...
 * @return none
 */
VOID_T tuya_mpu6050_power_on(_IN CONST gpio_pin_e pin, _IN CONST BOOL_T active_low)
{
    if (!sg_pwr_pin_used)
    {
        hal_gpio_pin_init(pin, FALSE);
        sg_pwr_pin_used = TRUE;
    }
    hal_gpio_write(pin, !active_low);
 ...
 * @return none
 */
VOID_T tuya_mpu6050_power_off(_IN CONST gpio_pin_e pin, _IN CONST BOOL_T active_low)
{
    if (!sg_pwr_pin_used)
    {
        hal_gpio_pin_init(pin, FALSE);
                          ...
                          _IN TY_GPIO_IRQ_CB int_cb)
{
    ...
    if (int_cb != NULL)
    {
        // tuya_gpio_init(pin, TRUE, TRUE);
        // hal_gpio_wakeup_set(pin, IO_Wakeup_Pol_e type);
        hal_gpio_pull_set(pin, GPIO_PULL_UP);
        // if (hal_gpioin_register(pin, NULL, int_cb))
