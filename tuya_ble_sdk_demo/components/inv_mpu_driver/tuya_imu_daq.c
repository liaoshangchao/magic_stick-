...
// STATIC VOID_T __new_data_ready_cb(gpio_pin_e pin, gpio_polarity_e type)
...
//    tuya_mpu6050_power_on(MPU_CT_POW_PIN, TRUE);
    ...
    MPU_RET ret = tuya_mpu6050_init(MPU_CLK_PLL_XGYRO, MPU_GYRO_FS_2000, MPU_ACCEL_FS_16, 1000 / DAQ_TIME_MS, MPU_INT_PIN, TY_GPIO_IRQ_FALLING, NULL);
    ...
    {
        TUYA_APP_LOG_INFO("tuya_mpu6050_init statue: %d", ret);
    }
    tuya_ble_timer_create(&daq_timer, DAQ_TIME_MS, TUYA_BLE_TIMER_REPEATED, __new_data_ready_cb);
...
{
    tuya_ble_timer_stop(daq_timer);
    tuya_ble_timer_delete(daq_timer);
