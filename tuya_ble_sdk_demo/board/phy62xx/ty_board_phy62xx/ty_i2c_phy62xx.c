 ...
 * LOCAL CONSTANT
 */
#define IIC_SDA_PIN GPIO_P26
#define IIC_SCL_PIN GPIO_P31
...
static void phy_i2c_DeInit(void)
{
    ...
    hal_gpio_pull_set(IIC_SDA_PIN, GPIO_FLOATING); // P26 -- SDA
    hal_gpio_pull_set(IIC_SCL_PIN, GPIO_FLOATING); // P31 -- SCL
}
...
static void i2c_sda_pin_mode_set(uint8_t mode, uint8_t level)
{
    if (mode == GPIO_INPUT_MODE)
    {
        ...
        hal_gpio_pin_init(IIC_SDA_PIN, GPIO_INPUT);
        hal_gpio_pull_set(IIC_SDA_PIN, GPIO_PULL_UP_S);
    }
    else if (mode == GPIO_OUTPUT_MODE)
    {
        //        nrf_gpio_cfg_output(TY_I2C_PIN_SDA);
        hal_gpio_pin_init(IIC_SDA_PIN, GPIO_OUTPUT);
        hal_gpio_pull_set(IIC_SDA_PIN, GPIO_FLOATING);
    }
}
...
void i2c_soft_gpio_init(void)
{
    ...
    hal_gpio_pin_init(IIC_SDA_PIN, GPIO_OUTPUT);
    hal_gpio_pin_init(IIC_SCL_PIN, GPIO_OUTPUT);
}
